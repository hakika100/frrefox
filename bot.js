...
client.on('ready', function(){
    var ms = 10000 ;
    var setGame = [' -help',`Im on ${client.guilds.size} servers`,`i help ${client.users.size} Members`,' [MAROC] Domstad RP V 1.0|| Is The Best ','a77777','lonly'];
    var i = -1;
    var j = 0;
...
client.on("message", (message) => {
   ...
   if (message.content.startsWith("-new")) {
        ...
        if (!message.guild.roles.exists("name", "giveme")) return message.channel.send(`This server doesn't have a \`giveme\` role made, so the ticket won't be opened.\nIf you are an administrator, make one with that name exactly and give it to users that should be able to see tickets.`);
        if (message.guild.channels.exists("name", "ticket-{message.author.id}" + message.author.id)) return message.channel.send(`You already have a ticket open.`);     
        message.guild.createChannel(`ticket-${message.author.username}`, "text").then(c => {
            let role = message.guild.roles.find("name", "giveme");
